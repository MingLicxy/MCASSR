...
import models
import utils
from test_ciaosr_mc_mat import eval_psnr, eval_psnr_fast
...
def train(train_loader, model, optimizer, epoch):
    ...
    #TODO 这里的两个参数为tensorboard服务
    num_dataset = 1000        # 数据集中图像数目
    ...
    for batch in tqdm(train_loader, leave=False, desc='train'):
        ...
        pred = model(inp, batch['inp_hr_coord'], batch['inp_cell'], ref, ref_hr)
...
def main(config_, save_path):
    ...
    for epoch in range(epoch_start, epoch_max + 1):
        ...
        if (epoch_val is not None) and (epoch % epoch_val == 0):
            ...
            #TODO 这里PSNR的计算方式决定了最佳权重的选取
            val_res = eval_psnr(val_loader, model_,
                data_norm=config['data_norm'],
                eval_type=config.get('eval_type'),
                eval_bsize=config.get('eval_bsize'))
            
            #TODO 对应mc-sr-implicit-downsampled-fast
            # val_res = eval_psnr_fast(val_loader, model_,
            #     data_norm=config['data_norm'],
            #     eval_type=config.get('eval_type'),
            #     eval_bsize=config.get('eval_bsize'))
