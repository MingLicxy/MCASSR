import argparse
import os
import math
from functools import partial

import yaml
import torch
from torch.utils.data import DataLoader
from tqdm import tqdm

import datasets
import models
import utils
################################# test-mc #######################################


#TODO 批量预测函数（需要针对mc的多输入重写）
def batched_predict(model, inp, coord, cell, ref, ref_hr, bsize):
    #print('############################################',coord.shape)  # 打印coord的形状
    #print('############################################',cell.shape)   # 打印cell的形状

    with torch.no_grad():
        model.gen_feat(inp, ref, ref_hr) # 获取latent code
        n = coord.shape[1] # 总共查询n个位置坐标
        ql = 0
        preds = []
        while ql < n:
            qr = min(ql + bsize, n) # 保证最后一个batch不超过n
            #TODO pred = model(inp, coord, cell)相当于调用了model.forward()
            #pred = model.query_rgb(coord[:, ql: qr, :], cell[:, ql: qr, :]) # 查询RGB
            #pred, ref_loss = model(inp, coord[:, ql: qr, :], cell[:, ql: qr, :], ref, ref_hr)
            pred = model(inp, coord[:, ql: qr, :], cell[:, ql: qr, :], ref, ref_hr)
            preds.append(pred)
            ql = qr
        pred = torch.cat(preds, dim=1)
    return pred


#TODO 重写该函数配合基于幅值的指标计算，在train中有引用
def eval_psnr(loader,
              model,
              data_norm=None,
              eval_type=None,
              eval_bsize=None,
              scale_max=4,
              verbose=False,
              mcell=False):
    model.eval()
    # 数据归一化
    if data_norm is None:
        data_norm = {
            'inp': {'sub': [0], 'div': [1]},
            'ref': {'sub': [0], 'div': [1]},
            'gt': {'sub': [0], 'div': [1]}
        }
    t = data_norm['inp']
    inp_sub = torch.FloatTensor(t['sub']).view(1, -1, 1, 1).cuda()
    inp_div = torch.FloatTensor(t['div']).view(1, -1, 1, 1).cuda()
    t = data_norm['ref']
    ref_sub = torch.FloatTensor(t['sub']).view(1, -1, 1, 1).cuda()
    ref_div = torch.FloatTensor(t['div']).view(1, -1, 1, 1).cuda()
    t = data_norm['gt']
    gt_sub = torch.FloatTensor(t['sub']).view(1, 1, -1).cuda()
    gt_div = torch.FloatTensor(t['div']).view(1, 1, -1).cuda()
    t = data_norm['ref']
    ref_hr_sub = torch.FloatTensor(t['sub']).view(1, 1, -1).cuda()
    ref_hr_div = torch.FloatTensor(t['div']).view(1, 1, -1).cuda()

    # eval_type决定评价数据集以及评价指标（eg:div2k-4）
    if eval_type is None: # 默认计算PSNR
        metric_fn = utils.calc_psnr
    #TODO 根据eval_type的值决定输入metric_fn中的参数
    elif eval_type.startswith('div2k'):
        scale = int(eval_type.split('-')[1])
        metric_fn = partial(utils.calc_psnr, dataset='div2k', scale=scale)
    elif eval_type.startswith('benchmark'):
        scale = int(eval_type.split('-')[1])
        metric_fn = partial(utils.calc_psnr, dataset='benchmark', scale=scale)
    elif eval_type.startswith('ssim'): # 通过改动eval_type计算SSIM
        #log_name = 'SSIM.txt'
        #scale = int(eval_type.split('-')[1])
        metric_fn = utils.ssim
    elif eval_type.startswith('mse'):
        metric_fn = utils.mse
    # 训练时也需要指定eval_type用于验证阶段选取最佳权重
    elif eval_type.startswith('kspace'):
        metric_fn = utils.calc_save
    else:
        raise NotImplementedError


    #TODO 用于计算所有验证批次的平均评价指标
    val_res = utils.Averager()
    

    ################################# 测试遍历 #####################################
    pbar = tqdm(loader, leave=False, desc='val')
    count = 0 # 设置计数器
    for batch in pbar:
        count += 1
        for k, v in batch.items():
            batch[k] = v.cuda(non_blocking=True)

        inp = (batch['inp'] - inp_sub) / inp_div
        ref = (batch['ref'] - ref_sub) / ref_div
        ref_hr = (batch['ref_hr'] - ref_hr_sub) / ref_hr_div

        coord = batch['inp_hr_coord']
        cell = batch['inp_cell']

        #TODO SRNO
        if mcell == False: c = 1
        else : c = max(scale/scale_max, 1)

        
        #TODO eval_bsize决定是否批量预测
        if eval_bsize is None:
            with torch.no_grad():
                #pred, ref_loss = model(inp, coord, cell*c, ref, ref_hr)
                pred = model(inp, coord, cell*c, ref, ref_hr)
        else:
            pred = batched_predict(model, inp, coord, cell*c, ref, ref_hr, eval_bsize)
        pred = pred * gt_div + gt_sub
        #print(pred.dtype)
        pred.clamp_(0, 1)

        # 为评价指标的计算reshape pred和batch['gt']
        if eval_type is not None: # reshape for shaving-eval
            ih, iw = batch['inp'].shape[-2:]
            s = math.sqrt(batch['inp_hr_coord'].shape[1] / (ih * iw))
            shape = [batch['inp'].shape[0], round(ih * s), round(iw * s), 2]
            pred = pred.view(*shape) \
                .permute(0, 3, 1, 2).contiguous()
            batch['gt'] = batch['gt'].view(*shape) \
                .permute(0, 3, 1, 2).contiguous()
        
        
        if eval_type.startswith('kspace'):
           # 计算得到三个指标，训练阶段只采用PSNR
           psnr_res, ssim_res, mse_res = metric_fn(batch['inp'], pred, batch['gt'])
           val_res.add(psnr_res, inp.shape[0])
           if verbose:
              pbar.set_description('val {:.4f}'.format(psnr_res))
        else:
            psnr_res = metric_fn(pred, batch['gt'])
            val_res.add(psnr_res.item(), inp.shape[0])
            if verbose:
               pbar.set_description('val {:.4f}'.format(psnr_res.item()))
        
    return val_res.item()



#TODO 对应mc-sr-implicit-downsampled-fast数据处理方式
def eval_psnr_fast(loader,
              model,
              data_norm=None,
              eval_type=None,
              eval_bsize=None,
              scale_max=4,
              verbose=False,
              mcell=False):
    model.eval()
    # 数据归一化
    if data_norm is None:
        data_norm = {
            'inp': {'sub': [0], 'div': [1]},
            'ref': {'sub': [0], 'div': [1]},
            'gt': {'sub': [0], 'div': [1]}
        }
    t = data_norm['inp']
    inp_sub = torch.FloatTensor(t['sub']).view(1, -1, 1, 1).cuda()
    inp_div = torch.FloatTensor(t['div']).view(1, -1, 1, 1).cuda()
    t = data_norm['ref']
    ref_sub = torch.FloatTensor(t['sub']).view(1, -1, 1, 1).cuda()
    ref_div = torch.FloatTensor(t['div']).view(1, -1, 1, 1).cuda()
    t = data_norm['gt']
    gt_sub = torch.FloatTensor(t['sub']).view(1, 1, -1).cuda()
    gt_div = torch.FloatTensor(t['div']).view(1, 1, -1).cuda()
    t = data_norm['ref']
    ref_hr_sub = torch.FloatTensor(t['sub']).view(1, 1, -1).cuda()
    ref_hr_div = torch.FloatTensor(t['div']).view(1, 1, -1).cuda()

    # eval_type决定评价数据集以及评价指标（eg:div2k-4）
    if eval_type is None: # 默认计算PSNR
        metric_fn = utils.calc_psnr
    #TODO 根据eval_type的值决定输入metric_fn中的参数
    elif eval_type.startswith('div2k'):
        scale = int(eval_type.split('-')[1])
        metric_fn = partial(utils.calc_psnr, dataset='div2k', scale=scale)
    elif eval_type.startswith('benchmark'):
        scale = int(eval_type.split('-')[1])
        metric_fn = partial(utils.calc_psnr, dataset='benchmark', scale=scale)
    elif eval_type.startswith('ssim'): # 通过改动eval_type计算SSIM
        #log_name = 'SSIM.txt'
        #scale = int(eval_type.split('-')[1])
        metric_fn = utils.ssim
    elif eval_type.startswith('mse'):
        metric_fn = utils.mse
    # 训练时也需要指定eval_type用于验证阶段选取最佳权重
    elif eval_type.startswith('kspace'):
        metric_fn = utils.calc_save
    else:
        raise NotImplementedError


    #TODO 用于计算所有验证批次的平均评价指标
    val_res = utils.Averager()
    

    ################################# 测试遍历 #####################################
    pbar = tqdm(loader, leave=False, desc='val')
    count = 0 # 设置计数器
    for batch in pbar:
        count += 1
        for k, v in batch.items():
            batch[k] = v.cuda(non_blocking=True)

        inp = (batch['inp'] - inp_sub) / inp_div
        ref = (batch['ref'] - ref_sub) / ref_div
        ref_hr = (batch['ref_hr'] - ref_hr_sub) / ref_hr_div

        coord = batch['inp_hr_coord']
        cell = batch['inp_cell']

        #TODO SRNO
        if mcell == False: c = 1
        else : c = max(scale/scale_max, 1)

        
        #TODO eval_bsize决定是否批量预测
        if eval_bsize is None:
            with torch.no_grad():
                #pred, ref_loss = model(inp, coord, cell*c, ref, ref_hr)
                pred = model(inp, coord, cell*c, ref, ref_hr)
        else:
            pred = batched_predict(model, inp, coord, cell*c, ref, ref_hr, eval_bsize)
        pred = pred * gt_div + gt_sub
        #print(pred.dtype)
        pred.clamp_(0, 1)

        #TODO 为评价指标的计算reshape pred和batch['gt']
        # if eval_type is not None: # reshape for shaving-eval
        #     ih, iw = batch['inp'].shape[-2:]
        #     s = math.sqrt(batch['inp_hr_coord'].shape[1] / (ih * iw))
        #     shape = [batch['inp'].shape[0], round(ih * s), round(iw * s), 2]
        #     pred = pred.view(*shape) \
        #         .permute(0, 3, 1, 2).contiguous()
        #     batch['gt'] = batch['gt'].view(*shape) \
        #         .permute(0, 3, 1, 2).contiguous()
        
        
        if eval_type.startswith('kspace'):
           # 计算得到三个指标，训练阶段只采用PSNR
           psnr_res, ssim_res, mse_res = metric_fn(batch['inp'], pred, batch['gt'])
           val_res.add(psnr_res, inp.shape[0])
           if verbose:
              pbar.set_description('val {:.4f}'.format(psnr_res))
        else:
            psnr_res = metric_fn(pred, batch['gt'])
            val_res.add(psnr_res.item(), inp.shape[0])
            if verbose:
               pbar.set_description('val {:.4f}'.format(psnr_res.item()))
        
    return val_res.item()


#TODO 重写该函数配合基于幅值的指标计算（这里定义计算PSNR SSIM MSE的方法）
def eval_metrics(loader,
                 model,
                 data_norm=None,
                 eval_type=None,
                 eval_bsize=None,
                 scale_max=4,
                 verbose=False,
                 mcell=False):
    model.eval()

    if data_norm is None:
        data_norm = {
            'inp': {'sub': [0], 'div': [1]},
            'ref': {'sub': [0], 'div': [1]},
            'gt': {'sub': [0], 'div': [1]}
        }
    t = data_norm['inp']
    inp_sub = torch.FloatTensor(t['sub']).view(1, -1, 1, 1).cuda()
    inp_div = torch.FloatTensor(t['div']).view(1, -1, 1, 1).cuda()
    t = data_norm['ref']
    ref_sub = torch.FloatTensor(t['sub']).view(1, -1, 1, 1).cuda()
    ref_div = torch.FloatTensor(t['div']).view(1, -1, 1, 1).cuda()
    t = data_norm['gt']
    gt_sub = torch.FloatTensor(t['sub']).view(1, 1, -1).cuda()
    gt_div = torch.FloatTensor(t['div']).view(1, 1, -1).cuda()
    t = data_norm['ref']
    ref_hr_sub = torch.FloatTensor(t['sub']).view(1, 1, -1).cuda()
    ref_hr_div = torch.FloatTensor(t['div']).view(1, 1, -1).cuda()

    # eval_type决定评价数据集以及评价指标（eg:div2k-4）
    if eval_type is None:
        metric_fn = utils.calc_psnr
    #TODO 根据eval_type的值决定输入metric_fn中的参数
    elif eval_type.startswith('div2k'):
        scale = int(eval_type.split('-')[1])
        metric_fn = partial(utils.calc_psnr, dataset='div2k', scale=scale)
    elif eval_type.startswith('benchmark'):
        scale = int(eval_type.split('-')[1])
        metric_fn = partial(utils.calc_psnr, dataset='benchmark', scale=scale)
    elif eval_type.startswith('ssim'): # 通过改动eval_type计算SSIM
        #log_name = 'SSIM.txt'
        #scale = int(eval_type.split('-')[1])
        metric_fn = utils.ssim
    elif eval_type.startswith('mse'):
        metric_fn = utils.mse

    #TODO ['metrics-2', 'metrics-3', 'metrics-4'......] 计算所有三个指标
    elif eval_type.startswith('metrics'):
        scale = int(eval_type.split('-')[1])
        psnr_fn = partial(utils.calc_psnr, dataset='div2k', scale=scale)
        ssim_fn = utils.ssim
        mse_fn = utils.mse
    # 采用复数幅值计算指标
    elif eval_type.startswith('kspace'):
        metric_fn = utils.calc_save # 返回psnr,ssim,mse同时保存可视化结果
    else:
        raise NotImplementedError


    #TODO 用于计算所有验证批次的平均评价指标
    val_res_psnr = utils.Averager()
    val_res_ssim = utils.Averager()
    val_res_mse = utils.Averager()

    ################################# 测试遍历 #####################################
    pbar = tqdm(loader, leave=False, desc='val')
    count = 0 # 设置计数器
    for batch in pbar:
        count += 1
        for k, v in batch.items():
            batch[k] = v.cuda(non_blocking=True)
        
        # 输入模型前进行归一化
        inp = (batch['inp'] - inp_sub) / inp_div # inp_lr
        ref = (batch['ref'] - ref_sub) / ref_div # ref_lr
        ref_hr = (batch['ref_hr'] - ref_hr_sub) / ref_hr_div # ref_hr

        coord = batch['inp_hr_coord']
        cell = batch['inp_cell']

        #TODO SRNO
        if mcell == False: c = 1
        else : c = max(scale/scale_max, 1)

        
        #TODO eval_bsize决定是否批量预测
        if eval_bsize is None:
            with torch.no_grad():
                #pred, ref_loss = model(inp, coord, cell*c, ref, ref_hr)
                pred = model(inp, coord, cell*c, ref, ref_hr)
        else:
            pred = batched_predict(model, inp, coord, cell*c, ref, ref_hr, eval_bsize)
        
        # 对模型的预测结果去归一化
        pred = pred * gt_div + gt_sub
        pred.clamp_(0, 1)

        # 为评价指标的计算reshape pred和batch['gt']
        if eval_type is not None: # reshape for shaving-eval
            ih, iw = batch['inp'].shape[-2:]   # [batch, 2, h, w]
            s = math.sqrt(batch['inp_hr_coord'].shape[1] / (ih * iw))
            #BUG .mat注意通道数为2
            shape = [batch['inp'].shape[0], round(ih * s), round(iw * s), 2] 
            pred = pred.view(*shape) \
                .permute(0, 3, 1, 2).contiguous() # [batch, 2, h, w]
            batch['gt'] = batch['gt'].view(*shape) \
                .permute(0, 3, 1, 2).contiguous() # [batch, 2, h, w]

        ######################## TODO 由eval_type决定如何计算指标 ########################
        if eval_type.startswith('metrics'):
            psnr_res = psnr_fn(pred, batch['gt'])
            ssim_res = ssim_fn(pred, batch['gt'])
            mse_res = mse_fn(pred, batch['gt'])

            val_res_psnr.add(psnr_res.item(), inp.shape[0])
            val_res_ssim.add(ssim_res.item(), inp.shape[0])
            val_res_mse.add(mse_res.item(), inp.shape[0])

            if verbose:
               pbar.set_description('val {:.4f}'.format(psnr_res.item()))

        elif eval_type.startswith('kspace'):
            # 不在这里保存测试结果
            psnr_res, ssim_res, mse_res = metric_fn(batch['inp'], pred, batch['gt'])

            val_res_psnr.add(psnr_res, inp.shape[0])
            val_res_ssim.add(ssim_res, inp.shape[0])
            val_res_mse.add(mse_res, inp.shape[0])

            if verbose:
               pbar.set_description('val {:.4f}'.format(psnr_res))
        ################################################################################

    return val_res_psnr.item(), val_res_ssim.item(), val_res_mse.item()




#TODO 对应mc-sr-implicit-downsampled-fast数据处理方式
def eval_metrics_fast(loader,
                 model,
                 data_norm=None,
                 eval_type=None,
                 eval_bsize=None,
                 scale_max=4,
                 verbose=False,
                 mcell=False):
    model.eval()

    if data_norm is None:
        data_norm = {
            'inp': {'sub': [0], 'div': [1]},
            'ref': {'sub': [0], 'div': [1]},
            'gt': {'sub': [0], 'div': [1]}
        }
    t = data_norm['inp']
    inp_sub = torch.FloatTensor(t['sub']).view(1, -1, 1, 1).cuda()
    inp_div = torch.FloatTensor(t['div']).view(1, -1, 1, 1).cuda()
    t = data_norm['ref']
    ref_sub = torch.FloatTensor(t['sub']).view(1, -1, 1, 1).cuda()
    ref_div = torch.FloatTensor(t['div']).view(1, -1, 1, 1).cuda()
    t = data_norm['gt']
    gt_sub = torch.FloatTensor(t['sub']).view(1, 1, -1).cuda()
    gt_div = torch.FloatTensor(t['div']).view(1, 1, -1).cuda()
    t = data_norm['ref']
    ref_hr_sub = torch.FloatTensor(t['sub']).view(1, 1, -1).cuda()
    ref_hr_div = torch.FloatTensor(t['div']).view(1, 1, -1).cuda()

    # eval_type决定评价数据集以及评价指标（eg:div2k-4）
    if eval_type is None:
        metric_fn = utils.calc_psnr
    #TODO 根据eval_type的值决定输入metric_fn中的参数
    elif eval_type.startswith('div2k'):
        scale = int(eval_type.split('-')[1])
        metric_fn = partial(utils.calc_psnr, dataset='div2k', scale=scale)
    elif eval_type.startswith('benchmark'):
        scale = int(eval_type.split('-')[1])
        metric_fn = partial(utils.calc_psnr, dataset='benchmark', scale=scale)
    elif eval_type.startswith('ssim'): # 通过改动eval_type计算SSIM
        #log_name = 'SSIM.txt'
        #scale = int(eval_type.split('-')[1])
        metric_fn = utils.ssim
    elif eval_type.startswith('mse'):
        metric_fn = utils.mse

    #TODO ['metrics-2', 'metrics-3', 'metrics-4'......] 计算所有三个指标
    elif eval_type.startswith('metrics'):
        scale = int(eval_type.split('-')[1])
        psnr_fn = partial(utils.calc_psnr, dataset='div2k', scale=scale)
        ssim_fn = utils.ssim
        mse_fn = utils.mse
    # 采用复数幅值计算指标
    elif eval_type.startswith('kspace'):
        metric_fn = utils.calc_save # 返回psnr,ssim,mse同时保存可视化结果
    else:
        raise NotImplementedError


    #TODO 用于计算所有验证批次的平均评价指标
    val_res_psnr = utils.Averager()
    val_res_ssim = utils.Averager()
    val_res_mse = utils.Averager()

    ################################# 测试遍历 #####################################
    pbar = tqdm(loader, leave=False, desc='val')
    count = 0 # 设置计数器
    for batch in pbar:
        count += 1
        for k, v in batch.items():
            batch[k] = v.cuda(non_blocking=True)
        
        # 输入模型前进行归一化
        inp = (batch['inp'] - inp_sub) / inp_div # inp_lr
        ref = (batch['ref'] - ref_sub) / ref_div # ref_lr
        ref_hr = (batch['ref_hr'] - ref_hr_sub) / ref_hr_div # ref_hr

        coord = batch['inp_hr_coord']
        cell = batch['inp_cell']

        #TODO SRNO
        if mcell == False: c = 1
        else : c = max(scale/scale_max, 1)

        
        #TODO eval_bsize决定是否批量预测
        if eval_bsize is None:
            with torch.no_grad():
                #pred, ref_loss = model(inp, coord, cell*c, ref, ref_hr)
                pred = model(inp, coord, cell*c, ref, ref_hr)
        else:
            pred = batched_predict(model, inp, coord, cell*c, ref, ref_hr, eval_bsize)
        
        # 对模型的预测结果去归一化
        pred = pred * gt_div + gt_sub
        pred.clamp_(0, 1)

        #TODO 为评价指标的计算reshape pred和batch['gt']
        # if eval_type is not None: # reshape for shaving-eval
        #     ih, iw = batch['inp'].shape[-2:]   # [batch, 2, h, w]
        #     s = math.sqrt(batch['inp_hr_coord'].shape[1] / (ih * iw))
        #     #BUG .mat注意通道数为2
        #     shape = [batch['inp'].shape[0], round(ih * s), round(iw * s), 2] 
        #     pred = pred.view(*shape) \
        #         .permute(0, 3, 1, 2).contiguous() # [batch, 2, h, w]
        #     batch['gt'] = batch['gt'].view(*shape) \
        #         .permute(0, 3, 1, 2).contiguous() # [batch, 2, h, w]

        ######################## TODO 由eval_type决定如何计算指标 ########################
        if eval_type.startswith('metrics'):
            psnr_res = psnr_fn(pred, batch['gt'])
            ssim_res = ssim_fn(pred, batch['gt'])
            mse_res = mse_fn(pred, batch['gt'])

            val_res_psnr.add(psnr_res.item(), inp.shape[0])
            val_res_ssim.add(ssim_res.item(), inp.shape[0])
            val_res_mse.add(mse_res.item(), inp.shape[0])

            if verbose:
               pbar.set_description('val {:.4f}'.format(psnr_res.item()))

        elif eval_type.startswith('kspace'):
            # 不在这里保存测试结果
            psnr_res, ssim_res, mse_res = metric_fn(batch['inp'], pred, batch['gt'])

            val_res_psnr.add(psnr_res, inp.shape[0])
            val_res_ssim.add(ssim_res, inp.shape[0])
            val_res_mse.add(mse_res, inp.shape[0])

            if verbose:
               pbar.set_description('val {:.4f}'.format(psnr_res))
        ################################################################################

    return val_res_psnr.item(), val_res_ssim.item(), val_res_mse.item()

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--config')
    parser.add_argument('--model')
    parser.add_argument('--scale_max', default='4')
    parser.add_argument('--gpu', default='0')
    parser.add_argument('--mcell', default=False)
    args = parser.parse_args()

    os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu

    with open(args.config, 'r') as f:
        config = yaml.load(f, Loader=yaml.FullLoader)

    spec = config['test_dataset']
    dataset = datasets.make(spec['dataset'])
    dataset = datasets.make(spec['wrapper'], args={'dataset': dataset})
    #TODO 创建测试数据加载器
    loader = DataLoader(dataset, batch_size=spec['batch_size'],
        num_workers=8, pin_memory=True)

    model_spec = torch.load(args.model)['model']
    model = models.make(model_spec, load_sd=True).cuda()
    

    import time
    t1= time.time()
    #TODO 这里的输出的res是一个元组，res[0]=PSNR; res[1]=SSIM; res[2]=RMSE
    res = eval_metrics(loader, model,
        data_norm=config.get('data_norm'),
        eval_type=config.get('eval_type'),
        eval_bsize=config.get('eval_bsize'),
        scale_max = int(args.scale_max),
        verbose=True,
        mcell=bool(args.mcell)
        )
    t2 =time.time()
    # '#######################[div2k-x2]#######################'在测试脚本中设置的，四舍五入保留后四位
    print('$$$$$$$$$$$$$$$$$$$$$[result]$$$$$$$$$$$$$$$$$$$$$: PSNR: {:.4f}   SSIM: {:.4f}   MSE: {:.9f}'.format(res[0], res[1], res[2]), utils.time_text(t2-t1))
