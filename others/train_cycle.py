...
import models
import utils
from test_cycle import eval_psnr
...
def train(train_loader, model, optimizer, epoch):
    ...
    for batch in pbar:
        ...
        hr_pred, lr_pred = model(inp, batch['hr_coord'], batch['lr_coord'], batch['hr_cell'], batch['lr_cell'])

        hr_gt = (batch['hr_gt'] - gt_sub) / gt_div
        lr_gt = (batch['lr_gt'] - gt_sub) / gt_div
        #print('################## pred_type ###################', type(pred))
        #print('################## gt_type ###################', type(gt))

        #TODO 损失计算（采用cycle_loss）
        loss = loss_fn(hr_pred, hr_gt, lr_pred, lr_gt)
        psnr = metric_fn(hr_pred, hr_gt)
