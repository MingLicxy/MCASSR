...
#TODO 批量预测函数（由于HR和LR的坐标对应不明确，最好不要采用batched_predict）
def batched_predict(model, inp, hr_coord, hr_cell, bsize):
    #print('############################################',coord.shape)  # 打印coord的形状
    #print('############################################',cell.shape)   # 打印cell的形状

    with torch.no_grad():
        model.gen_feat(inp) # 获取latent code
        n = hr_coord.shape[1] # 总共查询n个位置坐标
        ql = 0
        preds = []
        while ql < n:
            ...
            pred = model(inp, hr_coord[:, ql: qr, :], hr_cell[:, ql: qr, :], phase='testing')
            preds.append(pred)
            ql = qr
...
def eval_psnr(loader, model, data_norm=None, eval_type=None, eval_bsize=None, scale_max=4,
    ...
    for batch in pbar:
        ...
        lr_coord = batch['lr_coord']
        hr_cell = batch['hr_cell']
        lr_cell = batch['lr_cell']
        ...
        if eval_bsize is None:
            with torch.no_grad():
                 pred, _ = model(inp, hr_coord, hr_cell*c, phase='testing')
        else:
             pred = batched_predict(model, inp, hr_coord, hr_cell*c, eval_bsize)
        pred = pred * gt_div + gt_sub
        pred.clamp_(0, 1)

        # 为评价指标的计算reshape pred和batch['hr_gt']
        if eval_type is not None: # reshape for shaving-eval
            ...
            batch['hr_gt'] = batch['hr_gt'].view(*shape) \
                .permute(0, 3, 1, 2).contiguous()
        
        res = metric_fn(pred, batch['hr_gt'])
...
def eval_metrics(loader, model, data_norm=None, eval_type=None, eval_bsize=None, scale_max=4,
    ...
    for batch in pbar:
        ...
        if eval_type is not None: # reshape for shaving-eval
            ih, iw = batch['inp'].shape[-2:]
            s = math.sqrt(batch['hr_coord'].shape[1] / (ih * iw))
            shape = [batch['inp'].shape[0], round(ih * s), round(iw * s), 3]
            pred = pred.view(*shape) \
                .permute(0, 3, 1, 2).contiguous()
            batch['hr_gt'] = batch['hr_gt'].view(*shape) \
        ...
        psnr_res = psnr_fn(pred, batch['hr_gt'])
        ssim_res = ssim_fn(pred, batch['hr_gt'])
        mse_res = mse_fn(pred, batch['hr_gt'])
