...
def eval_metrics(loader, model, data_norm=None, eval_type=None, eval_bsize=None, scale_max=4,
    ...
    for batch in pbar:
        ...
        if eval_type is not None: # reshape for shaving-eval
            ...
            # batch['gt'] shape: [B, N, C]
            B, N, C = batch['gt'].shape

            # 原始输入尺寸
            ih, iw = batch['inp'].shape[-2:]

            # 根据坐标数量估算预测图像的 H, W
            H_pred = round(math.sqrt(N * ih / iw))
            W_pred = round(N / H_pred)

            # reshape pred 和 gt 到 [B, C, H, W]
            pred = pred.view(B, N, C).permute(0, 2, 1).contiguous()  # [B, C, N]
            pred = pred.view(B, C, H_pred, W_pred)

            batch['gt'] = batch['gt'].permute(0, 2, 1).contiguous()  # [B, C, N]
            batch['gt'] = batch['gt'].view(B, C, H_pred, W_pred)

            # 如果 H_pred/W_pred != GT 尺寸，再用插值对齐（可选）
            # pred = F.interpolate(pred, size=(ih*scale, iw*scale), mode='bicubic', align_corners=False)

        
        psnr_res = psnr_fn(pred, batch['gt'])
        ssim_res = ssim_fn(pred, batch['gt'])
        mse_res = mse_fn(pred, batch['gt'])
        lpips_res = lpips_fn(pred, batch['gt'])
        ...
        val_res_lpips.add(lpips_res.item(), inp.shape[0])

        if verbose:
            pbar.set_description('val {:.4f}'.format(psnr_res.item()))

    return val_res_psnr.item(), val_res_ssim.item(), val_res_mse.item(), val_res_lpips.item()
...
if __name__ == '__main__':
    ...
    print('$$$$$$$$$$$$$$$$$$$$$[result]$$$$$$$$$$$$$$$$$$$$$: PSNR: {:.4f}   SSIM: {:.4f}   MSE: {:.9f}  LPIPS: {:.4f}'.format(res[0], res[1], res[2], res[3]), utils.time_text(t2-t1))
