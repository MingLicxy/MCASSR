...
from losses.archs.vgg_arch import VGGFeatureExtractor
...
class FFTLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        #TODO GPU上处理频域转换会出现困难，可以转到CPU上进行操作
        pred_fft = torch.fft.rfft2(pred.cpu()).cuda()
        target_fft = torch.fft.rfft2(target.cpu()).cuda()
...
#TODO L1Loss与FFTLoss组合损失
class CombinedLoss(nn.Module):
    def __init__(self, weight_fft=0.05):
        super(CombinedLoss, self).__init__()
        self.l1_loss = nn.L1Loss()
        self.fft_loss = FFTLoss(weight_fft)

    def forward(self, input, target):
        l1_loss_value = self.l1_loss(input, target)
        fft_loss_value = self.fft_loss(input, target)
        return l1_loss_value + fft_loss_value
