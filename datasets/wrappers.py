...
from utils import to_pixel_samples, to_pixel_samples_mat
...
def kspace_crop_test(x, ref, s):
    """
    input:
        接受复数类型数组输入
        x: input HR complex matrix [H,W]
        ref: input HR complex matrix [H,W]
        s: 下采样因子(可能非整数)
    ...
    H, W = x.shape # x的原始尺寸
    h_lr = math.floor(H / s + 1e-9) # h_lr是整数
    ...
    w_hr = min(w_hr, W)
    # 普通裁剪
    # x_crop_hr = x[:h_hr, :w_hr]
    # ref_crop_hr = ref[:h_hr, :w_hr]
    #TODO 中心裁剪
    x_crop_hr = x[H//2-math.floor(h_hr/2):H//2+math.ceil(h_hr/2), W//2-math.floor(w_hr/2):W//2+math.ceil(w_hr/2)]
    ref_crop_hr = ref[H//2-math.floor(h_hr/2):H//2+math.ceil(h_hr/2), W//2-math.floor(w_hr/2):W//2+math.ceil(w_hr/2)]

    x_fs_crop_hr = fft2c(x_crop_hr) # 转换到频域（形状不变） 
    ref_fs_crop_hr = fft2c(ref_crop_hr)

    h_c,w_c = h_hr//2,w_hr//2 # x_crop_hr的中心坐标

    #TODO K-space裁剪下采样（中心裁剪）
    x_fs_crop_lr = x_fs_crop_hr[h_c-math.floor(h_lr/2):h_c+math.ceil(h_lr/2),w_c-math.floor(w_lr/2):w_c+math.ceil(w_lr/2)]
    ref_fs_crop_lr = ref_fs_crop_hr[h_c-math.floor(h_lr/2):h_c+math.ceil(h_lr/2),w_c-math.floor(w_lr/2):w_c+math.ceil(w_lr/2)]

    x_hr = x_crop_hr # x是复数类型，x_hr也是复数类型
    ref_hr = ref_crop_hr

    x_lr = ifft2c(x_fs_crop_lr) # 逆傅里叶变换输出复数类型
    ref_lr = ifft2c(ref_fs_crop_lr)
    ...
    ref_hr_real = ref_hr_real[ :, :,np.newaxis] # 增加一个维度 [H,W,1]
    ref_hr_imag = ref_hr.imag
    ref_hr_imag = ref_hr_imag[ :, :,np.newaxis]
    ref_hr = np.concatenate((ref_hr_real,ref_hr_imag),2) # [H,W,2] 浮点数类型
    ...
    ref_hr = np.ascontiguousarray(ref_hr.transpose(2, 0, 1))
    ref_hr = torch.from_numpy(ref_hr).float()
    ref_lr = np.ascontiguousarray(ref_lr.transpose(2, 0, 1))
    ref_lr = torch.from_numpy(ref_lr).float()

    return x_hr, x_lr, ref_hr, ref_lr # [2,H,W]的Tensor

###BUG x与ref在训练阶段一定要同步处理
def kspace_crop_train(x, ref, s, inp_size):
    """
    input:
        接受复数类型数组输入
        x: input HR complex matrix [H,W]
        ref: input HR complex matrix [H,W]
        s: 下采样因子(可能非整数)
        inp_size: 事先确定的h_lr=w_lr=inp_size
    ...
    #TODO 随机裁剪（x与ref随机裁剪起点相同）
    x0 = random.randint(0, H - h_hr)
    y0 = random.randint(0, W - w_hr)

    x_crop_hr = x[x0 : x0 + h_hr, y0 : y0 + w_hr]
    ref_crop_hr = ref[x0 : x0 + h_hr, y0 : y0 + w_hr]
    ...
    # 在通道维上cat实部与虚部
    x_hr_real = x_hr.real # 实数数组的实部就是本身
    x_hr_real = x_hr_real[ :, :,np.newaxis] # 增加一个维度 [H,W,1]
    x_hr_imag = x_hr.imag
    x_hr_imag = x_hr_imag[ :, :,np.newaxis]
    x_hr = np.concatenate((x_hr_real,x_hr_imag),2) # [H,W,2] 浮点数类型

    x_lr_real = x_lr.real # 实数数组的虚部就是全零
    x_lr_real = x_lr_real[ :, :,np.newaxis]
    x_lr_imag = x_lr.imag
    x_lr_imag = x_lr_imag[ :, :,np.newaxis]
    x_lr = np.concatenate((x_lr_real,x_lr_imag),2)
...
################################### TODO Dual-ArbNet-main ###################################
def paired_crop(x, ref, scale):
    """
    input:
        x: input HR complex matrix [H,W]
        lq_size: target LR shape
        scale: downsample scale of [x1,x2]
    output:
    ...
    x_hr_real = x_hr.real
    x_hr_real = x_hr_real[ :, :,np.newaxis] # 增加一个维度
    x_hr_imag = x_hr.imag
    x_hr_imag = x_hr_imag[ :, :,np.newaxis]
...
#TODO 训练时采用，out-of-scale测试时采用 （训练测试都采用双三次）
...
class MCSRImplicitDownsampled(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.inp_size is None: # 为测试做准备，模型对输入LR的尺寸有要求
            ...
            w_lr = self.inp_size
            w_hr = round(w_lr * s) # 训练时 64 x 4 = 256  round取整数
...
#TODO 训练以及测试阶段都采用K-space裁剪下采样
@register('mc-sr-implicit-downsampled-mat')
class MCSRImplicitDownsampledMat(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.inp_size is None: # 为测试做准备，模型对输入LR的尺寸有要求
            T2_crop_hr, T2_crop_lr, T1_crop_hr, T1_crop_lr = kspace_crop_test(T2_img, T1_img, s) # [2,H,W]Tensor
            # print("##########################################", T2_crop_lr.shape) # [3, 120, 112]
        else:
            ##################################### BUG 训练时随机裁剪的起点PD与T2要相同 #####################################
            T2_crop_hr, T2_crop_lr, T1_crop_hr, T1_crop_lr = kspace_crop_train(T2_img, T1_img, s, self.inp_size) # [2,H,W]Tensor
            # print("##########################################", T2_crop_lr.shape) # [2, 48, 48]
        ...
        ####### prepare inp #########
        #BUG to_pixel_samples()通道设置
        T2_hr_coord, T2_hr_rgb = to_pixel_samples_mat(T2_crop_hr.contiguous())

        ####### prepare ref #########
        _, T1_hr_rgb = to_pixel_samples_mat(T1_crop_hr.contiguous())
...
#TODO Dual-ArbNet数据处理方案
@register('mc-sr-implicit-downsampled-dual')
class MCSRImplicitDownsampledDual(Dataset):
    ...
    def __getitem__(self, idx):
        #TODO 一次取得两个对比度的MRI图像，实际上PD作为Ref
        T2_img, T1_img = self.dataset[idx] # [H,W]numpy
        
        #TODO train:[1,4]  test:[2,3,4,6,8,12] 公倍数: 24
        s = random.uniform(self.scale_min, self.scale_max) 
        
        #TODO 先进行下采样，再决定是否切片
        hr, lr, ref_hr, ref_lr = paired_crop(T2_img, T1_img, [s,s]) # [H,W,2]numpy
        lr_cat = np.concatenate([lr,ref_lr],axis=2) 

        # test
        if self.inp_size is None: # 为测试做准备，模型对输入LR的尺寸有要求
            ih, iw = lr_cat.shape[:2]
            hr = hr[0:int(ih * s), 0:int(iw * s)]
            ref_hr = ref_hr[0:int(ih * s), 0:int(iw * s)]
            lr_tensor, hr_tensor, ref_hr_tensor = common.np2Tensor(lr_cat, hr, ref_hr, rgb_range=1) # [2,H,W]tensor
            T2_crop_hr = hr_tensor
            T2_crop_lr = lr_tensor[:2,:,:]
            T1_crop_hr = ref_hr_tensor
            T1_crop_lr = lr_tensor[2:,:,:]
            
            # 对测试阶段数据进行预处理
            T2_crop_hr, T2_crop_lr, T1_crop_hr, T1_crop_lr = common.crop_border(T2_crop_hr, T2_crop_lr, T1_crop_hr, T1_crop_lr, s, s)
        # train   
        else:
            lr, hr, ref_hr = common.get_patch(lr_cat,
                                              hr,
                                              ref_hr,
                                              patch_size=self.inp_size,
                                              scale=s,
                                              scale2=s)
            lr_tensor, hr_tensor, ref_hr_tensor = common.np2Tensor(lr, hr, ref_hr, rgb_range=1) # [2,H,W]tensor
            T2_crop_hr = hr_tensor
            T2_crop_lr = lr_tensor[:2,:,:]
            T1_crop_hr = ref_hr_tensor
            T1_crop_lr = lr_tensor[2:,:,:]
        ...
        ####### prepare inp #########
        #BUG to_pixel_samples()用于[c,h,w]->[h*w,c]
        T2_hr_coord, T2_hr_rgb = to_pixel_samples_mat(T2_crop_hr.contiguous())
...
#TODO 适配SRNO以及DIINN等方法
@register('mc-sr-implicit-downsampled-fast')
class MCSRImplicitDownsampledFast(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #TODO 区别于非fast
        h_hr, w_hr = T2_crop_hr.shape[-2:] # 获取H,W
        h_lr, w_lr = T2_crop_lr.shape[-2:]
        T2_hr_coord = make_coord([h_hr, w_hr], flatten=False)
        T2_hr_rgb = T2_crop_hr # [c=3,h_hr,w_hr]
        T1_hr_rgb = T1_crop_hr

        #TODO 在HR中随机选择(h_lr*w_lr)个像素索引（作用同sample_q）
        if self.inp_size is not None:
            
            idx = torch.tensor(np.random.choice(h_hr*w_hr, h_lr*w_lr, replace=False))
            #idx,_ = torch.sort(idx)
            T2_hr_coord = T2_hr_coord.view(-1, T2_hr_coord.shape[-1])
            T2_hr_coord = T2_hr_coord[idx, :]
            T2_hr_coord = T2_hr_coord.view(h_lr, w_lr, T2_hr_coord.shape[-1]) # [h_lr,w_lr,2]

            T2_hr_rgb = T2_crop_hr.contiguous().view(T2_crop_hr.shape[0], -1)
            T1_hr_rgb = T1_crop_hr.contiguous().view(T1_crop_hr.shape[0], -1)
            T2_hr_rgb = T2_hr_rgb[:, idx]
            T1_hr_rgb = T1_hr_rgb[:, idx]

            T2_hr_rgb = T2_hr_rgb.view(T2_crop_hr.shape[0], h_lr, w_lr) # [c=3,h_lr,w_lr]
            T1_hr_rgb = T1_hr_rgb.view(T1_crop_hr.shape[0], h_lr, w_lr)
        #TODO cell张量维度是[2,]默认在bs维度上堆叠[bs,2]
        T2_cell = torch.tensor([2 / T2_crop_hr.shape[-2], 2 / T2_crop_hr.shape[-1]], dtype=torch.float32)
        
        return {
            'inp': T2_crop_lr, # [3, inp_size, inp_size]
            'inp_hr_coord': T2_hr_coord, # [inp_size, inp_size, 2]
            'inp_cell': T2_cell, # [2]
            'ref': T1_crop_lr,
            'ref_hr': T1_hr_rgb,
            'gt': T2_hr_rgb # [3, inp_size, inp_size]
        }  
...
if __name__== '__main__':
    ...
    x_complex = x.to(dtype=torch.cfloat)
    ref = torch.randn(256, 256)
    ref = torch.clamp(ref, min=0)
    ref_complex = ref.to(dtype=torch.cfloat)
    s = 4
    inp_size = 48

    output = kspace_crop_train(x_complex, ref_complex, s, inp_size)
    print(output[0].shape) # x_hr
    print(output[1].shape) # x_lr
    print(output[2].shape) # ref_hr
    print(output[3].shape) # ref_lr
