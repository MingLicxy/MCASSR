...
import torch
import cv2
from dl_utils.lam_utils import _add_batch_one
from dl_utils.lam_utils import make_coord
from dl_utils.lam_utils import interpolation, isotropic_gaussian_kernel
...
#TODO 存在模型推理过程
def Path_gradient(numpy_image, resolution, model, attr_objective, path_interpolation_func, cuda=True):
    """
    ...
    for i in tqdm(range(image_interpolation.shape[0])):
        ...
        if cuda:
            ...
            result = batched_predict(model, ((img_tensor - 0.5) / 0.5).cuda().unsqueeze(0), coord.unsqueeze(0), cell.unsqueeze(0), bsize=30000)[0]
            #print("#############################################", result.requires_grad) # False
            result = (result * 0.5 + 0.5).clamp(0, 1).view(h, w, 3).permute(2, 0, 1).unsqueeze(0)
            #print("#############################################", result.shape); # [3, 480, 640]
            ...
            result = batched_predict(model, ((img_tensor - 0.5) / 0.5).unsqueeze(0), coord.unsqueeze(0), cell.unsqueeze(0), bsize=30000)[0]
            result = (result * 0.5 + 0.5).clamp(0, 1).view(h, w, 3).permute(2, 0, 1).unsqueeze(0)
            target = attr_objective(result)
            target.backward()
