...
NEG_INF = -1000000 # 负无穷边界值
T_MAX = 128*128 #TODO 权衡RWKV性能与显存占用的重要超参数
#T_MAX = 512*512
# RWKV
...
def load_wkv_cuda(T_MAX):
    ...
    return load(
        ...
        extra_cuda_cflags=[
            '-res-usage', 
            '--maxrregcount 60', # 限制每个线程使用的最大寄存器数为60（与显存占用无关）
            '--use_fast_math', 
            '-O3', 
            '-Xptxas -O3', 
            f'-DTmax={T_MAX}'
...
class RWKV_CNN(nn.Module):
    def __init__(self,
        ...
        self.norm = norm_layer(self.rwkv_dim + self.cnn_dim) # 定义归一化层
        ...
        for i_layer in range(self.num_layers): # 4
            layer = nn.Sequential( #TODO 论文中残差连接未体现出来
                nn.LayerNorm(rwkv_dim), #TODO 注意层归一化
        ...
        if resi_connection == '1conv': #TODO 最后一层Conv负责将fusion_feat的维度降低
            self.conv_after_body = nn.Conv2d(fus_dim, rwkv_dim, 3, 1, 1)
        elif resi_connection == '3conv':
            self.conv_after_body = nn.Sequential(
                ...
                nn.Conv2d(fus_dim // 4, rwkv_dim, 3, 1, 1))
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_layers): # 4
            ...
            if i != (self.num_layers-1): #TODO 前(n-1)个融合模块
                ...
                x_rwkv, x_cnn = torch.split(x_fus, [self.rwkv_dim, self.cnn_dim], 1) # [bs, c, h, w]
                #TODO fusion_rwkv[i]中包含归一化层
