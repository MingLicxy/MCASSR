...
class LIIF_CYCLE(nn.Module):
    ...
    def forward(self, inp, hr_coord, hr_cell, lr_coord=None, lr_cell=None, phase='training'):
        """
        inp: LR
        hr_coord: HR coord grid
        hr_cell: HR cell
        lr_coord: LR coord grid
        lr_cell: LR cell
        phase: training/testing
        注意: 需要的lr_coord和lr_cell是通过数据处理得到还是在函数中实现
        """
        self.inp = inp 
        flr = self.gen_feat(inp) 
        hr = self.query_rgb(flr, hr_coord, hr_cell)
        #print('###############################################', hr.shape) # [2, 2304, 3] batch_size = 2
        #print('###############################################', inp.shape) # [2, 3, 48, 48]
        if phase == 'testing':
            return hr

        hr_resize = hr.permute(0, 2, 1).reshape(-1, 3, inp.shape[-2], inp.shape[-1])

        fhr = self.gen_feat(hr_resize) 
        # lr_coord = make_coord(flr.shape[-2:], flatten=False).cuda() \
        #     .permute(2, 0, 1) \
        #     .unsqueeze(0).expand(flr.shape[0], 2, *flr.shape[-2:])
        lr = self.query_rgb(fhr, lr_coord, lr_cell)
        #TODO 注意有两个输出（对应train_cycle和test_cycle中的代码也需要改动）
        return hr, lr
