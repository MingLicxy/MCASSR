...
from models.arch_utils.arch_util import to_2tuple, trunc_normal_
from models.arch_utils.ipg_kit import flex, cossim, local_sampling, global_sampling
import numpy as np
import einops
...
class IPG_Grapher(nn.Module):
    ...
    def get_rel_pos_index(self):
        ...
        coords_grid = torch.stack(torch.meshgrid([torch.arange(group_size[0]), torch.arange(group_size[1])], indexing='ij'))
        coords_grid_flatten = torch.flatten(coords_grid, 1)

        coords_sample = torch.stack(torch.meshgrid([torch.arange(sample_size[0]), torch.arange(sample_size[1])], indexing='ij'))
...
class MGB(nn.Module):
    ...
    def __init__(self,
        ...
        #TODO 包含sample_size的unfold_dict
    ...
    # 利用diff=|HR-bicubic(LR)|进行高低频区域划分
    ...
    @torch.no_grad()
    def calc_graph(self, x_, x_size): # [1, 24300=180*135, 180]

        #print('############################################', x_size) # [135, 180]
...
class IPG(nn.Module):
    ...
    def forward(self, x): # [1, 3, 135, 180]
        '''
...
@register('ipg')
def make_ipg(no_upsampling=True):
    #TODO 参数之间存在关联：img_size=64, sample_size=32, window_size=16
    #                     img_size=48, sample_size=24, window_size=12 
    model = IPG(
        in_chans=3,
        img_size=48, 
        window_size=12, #TODO 窗口尺寸是重要超参 16
        depths=[2, 2, 2, 2], # [6, 6, 6, 6, 6, 6]
        embed_dim=180,
        num_heads=[2, 2, 2, 2], # [6, 6, 6, 6, 6, 6]
        mlp_ratio=4,
        upsampler='none', #TODO 不进行上采样
        resi_connection='1conv',
        graph_flags=[1,1,1,1,1,1],
        stage_spec=[['GN','GS','GN','GS','GN','GS'],['GN','GS','GN','GS','GN','GS'],['GN','GS','GN','GS','GN','GS'],['GN','GS','GN','GS','GN','GS'],['GN','GS','GN','GS','GN','GS'],['GN','GS','GN','GS','GN','GS']],
        dist_type='cossim',
        top_k=256,
        head_wise=0,
        sample_size=24, #TODO 这里的参数可以在IPG模型类中进行默认定义（img_size需要被sample_size整除）
        graph_switch=1,
        flex_type='interdiff_plain',
        FFNtype='basic-dwconv3',
        conv_scale=0,
