...
class CAB(nn.Module):
    def __init__(self, num_feat, is_light_sr= False, compress_ratio=3,squeeze_factor=30):
        ...
        self.cab = nn.Sequential(
            #TODO MambaIR关键改进一：瓶颈Conv
            nn.Conv2d(num_feat, num_feat // compress_ratio, 3, 1, 1),
            nn.GELU(),
            nn.Conv2d(num_feat // compress_ratio, num_feat, 3, 1, 1),
            #TODO MambaIR关键改进二：CA   这里作为消融点
...
##################################------MambaIR------#################################
class Mamba_CNN_NO_CAtten(nn.Module):
    def __init__(self,
                 ...
                 resi_connection='1conv',
                 **kwargs):
        super(Mamba_CNN_NO_CAtten, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
...
@register('mamba_cnn_no_catten')
def make_mambair(no_upsampling=True):
    return Mamba_CNN_NO_CAtten()
