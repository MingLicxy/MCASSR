...
####################################### SeemoRe ######################################
# come from https://github.com/eduardzamfir/seemoredetails/blob/main/basicsr/archs/seemore_arch.py
...
class SeemoRe(nn.Module):
    def __init__(self,
                 scale: int = 1,
                 in_chans: int = 3,
                 num_experts: int = 6, # 专家数
                 num_layers: int = 6, # 残差组数
                 embedding_dim: int = 64, # 通道维
                 img_range: float = 1.0,
                 use_shuffle: bool = False,
                 global_kernel_size: int = 11,
                 recursive: int = 2,
                 lr_space: str = "linear",  #lr_space: int = 1, # [ "linear", "exp", "double"]
                 topk: int = 2,):
        super().__init__()
        self.scale = scale
...
class ResGroup(nn.Module):
    def __init__(self,
                 ...
                 num_experts: int,
                 global_kernel_size: int = 11,
                 lr_space: str = "linear",   #lr_space: int = 1,
                 topk: int = 2,
                 recursive: int = 2,
                 use_shuffle: bool = False):
        super().__init__()
...
class MoEBlock(nn.Module):
    def __init__(self,
        ...
        self.recursive = recursive #TODO 递归跨步卷积次数（用于获取特征金字塔）
        ...
        # TODO lr_space决定了不同专家的hidden_dim
        if lr_space == "linear":
...
class MoELayer(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, k: torch.Tensor):
        ...
        #TODO 这里区别训练与推理过程
        if self.training:
            # 将weights张量中特定位置的值复制到exp_weights张量中
...
if __name__== '__main__':
    ...
    model = SeemoRe(scale= 1,
                    in_chans=3,
                    num_experts=6, # 专家数
                    num_layers=6,
                    embedding_dim=64, # 通道维
                    img_range=1.0,
                    use_shuffle=False,
...
@register('seemore')
def make_seemore(no_upsampling=True):
    return SeemoRe()