...
class HAT(nn.Module):
    ...
    def __init__(self,
                 ...
                 in_chans=3,
                 embed_dim=180,
                 depths=(2, 2, 2, 2),   #(8, 8, 8, 8, 8)
                 num_heads=(2, 2, 2, 2),  #(6, 6, 6, 6, 6)
                 window_size=8,  #16   #TODO widow_size与shift_size应该有关联
                 shift_size = (0, 0, 8, 8, 16, 16, 24, 24),  #changed to tuple #BUG 是否与shift_size有关
                 compress_ratio=3,
                 squeeze_factor=30,
                 conv_scale=0.01,
                 overlap_ratio=0.5,
...
#######################---------Hybrid_Attention_Transformer(HAT)---------#######################
@register('hat')
def make_hat(no_upsampling=True):
    return HAT()
