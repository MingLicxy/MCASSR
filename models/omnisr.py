...
class ESA(nn.Module):
    ...
    def forward(self, x):
        ...
        print('#########################################################', c1.shape) # [1, 16, 3, 7] [bs, c, h, w]
        #TODO 当上采样尺度不同时c1张量的维度会发生改变，当其维度过小时会导致v_max为负数
...
class OSAG(nn.Module):
    ...
    def forward(self, x):
        ...
        out = out + x
        #TODO 出BUG的ESA是模型的最后一层（训练阶段最大下采样尺度就4，测试阶段有30）
        return self.esa(out)
##########################------OSAG------###########################
...
class OmniSR(nn.Module):
    def __init__(self,num_in_ch=3,num_out_ch=3,num_feat=64,**kwargs):
        ...
        res_num     = kwargs["res_num"] # OSAG层数
        up_scale    = kwargs["upsampling"] # 上采样尺度
        bias        = kwargs["bias"] # 如果使用了批量归一化，则设置为true
        upsampler        = kwargs["upsampler"] # 上采样器
...
@register('omnisr')
def make_omnisr(no_upsampling=True):
    model = OmniSR(
        num_in_ch=3,
        num_out_ch=3,
        num_feat=64,
