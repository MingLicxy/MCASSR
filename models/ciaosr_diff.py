...
采用差分注意力进行局部特征聚合，关注更相关的特征
潜在改进:
1,多头注意力是否必要
2,计算两个注意力图是否需要对query进行两次投影,只对key进行两次投影也能得到两个注意力图
3,差分注意力是否可以为更大的聚合范围做铺垫
"""
...
################################ CiaoSR_Diff ##################################
@register('ciaosr_diff')
class CiaoSR_Diff(nn.Module):
    """
    ...
    def __init__(self,
        ...
        # Q,K,V编码器（差分注意力）
        self.imnet_c = models.make(imnet_c, args={'in_dim': imnet_c_in_dim}) # 最终的解码器
        self.imnet_q1 = models.make(imnet_q, args={'in_dim': imnet_q_in_dim, 'out_dim': imnet_q_out_dim})
        self.imnet_q2 = models.make(imnet_q, args={'in_dim': imnet_q_in_dim, 'out_dim': imnet_q_out_dim})
        self.imnet_k1 = models.make(imnet_k, args={'in_dim': imnet_k_in_dim, 'out_dim': imnet_k_out_dim})
        self.imnet_k2 = models.make(imnet_k, args={'in_dim': imnet_k_in_dim, 'out_dim': imnet_k_out_dim})
        self.imnet_v = models.make(imnet_v, args={'in_dim': imnet_v_in_dim, 'out_dim': imnet_v_out_dim})

        # Init λ 
        self.lambda_q1 = nn.Parameter(torch.randn(1, imnet_q_out_dim) * 0.1)
        self.lambda_k1 = nn.Parameter(torch.randn(1, imnet_k_out_dim) * 0.1)
        self.lambda_q2 = nn.Parameter(torch.randn(1, imnet_q_out_dim) * 0.1)
        self.lambda_k2 = nn.Parameter(torch.randn(1, imnet_k_out_dim) * 0.1)
    ...
    def query_rgb(self, coord, scale=None):
        ...
        for feature in self.feats: #TODO 遍历的目的？
            ...
            #TODO 如果要对query投影后计算注意力，应该在循环外
            bs, q = coord.shape[:2]

            inp_q = torch.cat([query, scale_], dim=-1) # 不考虑相对坐标
            inp_q = inp_q.contiguous().view(bs * q, -1)
            weight_q1 = self.imnet_q1(inp_q).view(bs, q, -1).contiguous()  
            pred_q1 = (query * weight_q1).view(bs, q, -1).unsqueeze(-2) # [16, 2304, 1, 576]
            weight_q2 = self.imnet_q2(inp_q).view(bs, q, -1).contiguous()  
            pred_q2 = (query * weight_q2).view(bs, q, -1).unsqueeze(-2) # [16, 2304, 1, 576]
            ...
            for v in v_lst:
                ...
                # 利用meta-learning的思想（project）循环当中只算kv
                weight_k1 = self.imnet_k1(inp_k).view(bs, q, -1).contiguous()  # [16, 2304, 576]
                pred_k1 = (key * weight_k1).view(bs, q, -1)  # [16, 2304, 576]
                weight_k2 = self.imnet_k2(inp_k).view(bs, q, -1).contiguous()  # [16, 2304, 576]
                pred_k2 = (key * weight_k2).view(bs, q, -1)  # [16, 2304, 576]

                weight_v = self.imnet_v(inp_v).view(bs, q, -1).contiguous()  # [16, 2304, 576]
                pred_v = (value * weight_v).view(bs, q, -1)  # [16, 2304, 576]
            
                preds_k1.append(pred_k1)
                preds_k2.append(pred_k2)
                preds_v.append(pred_v)

            # batch-size=16, local_size=2（四个采样点）
            preds_k1 = torch.stack(preds_k1, dim=-1)  # [16, 2304, 576, 4]
            preds_k2 = torch.stack(preds_k2, dim=-1) # [16, 2304, 576, 4]
            preds_v = torch.stack(preds_v, dim=-2)  # [16, 2304, 4, 576]

            #TODO 利用注意力机制进行特征聚合
            attn1 = torch.matmul(pred_q1 ,preds_k1) / self.softmax_scale  # [16, 2304, 1, 4]
            attn2 = torch.matmul(pred_q2 ,preds_k2) / self.softmax_scale 
            attn1 = attn1.softmax(dim=-1)
            attn2 = attn2.softmax(dim=-1) 

            #print("#######################################################", attn1.shape) [16, 2304, 1, 4]
            ...
            #print("#######################################################", lambda_full.shape) [1, 1, 1]

            attn = attn1 - lambda_full * attn2
            x = torch.matmul(attn ,preds_v).view(bs * q, -1)  # [16*2304, 576]
