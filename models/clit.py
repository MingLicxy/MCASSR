...
class CLIT(nn.Module):
    def __init__(
        ...
        self.head = head
        self.r = r
        self.imnet_num = imnet_num # 与累积训练策略相关
    ...
    def query_rgb(self, sample_coord, cell=None):
        ...
        if self.local_attn:
            ...
            # [b, c, q, 49] -> [b, q, 49, c] -> [b, q, 49, head, c] -> [b, q, head, c, 49]
            # 通过最邻近插值获取每个查询点对应局部窗口中采样点的特征K
    ...
    def cascaded_forward(self, inp, coords, sample_coord, cell):
        ...
        for idx in range(len(coords)):
            ...
            if idx < len(coords) - 1:
                ...
                # 91行根据prev_feat更新self.feat（影响query_rgb）
            ...
            #self.im_idx += 1
            #self.conv_idx += 1
            if self.im_idx < self.imnet_num-1:
               self.im_idx += 1
            if self.conv_idx < self.conv_num-1:
               self.conv_idx += 1
