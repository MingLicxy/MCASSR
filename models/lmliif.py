...
class LMLIIF(nn.Module):
    ...
    def __init__(self,
                 encoder_spec,
                 imnet_spec=None, # Render MLP
                 hypernet_spec=None, # Latent MLP
                 local_ensemble=True,
                 feat_unfold=True,
                 cell_decode=True,
                 mod_input=False, #
        ...
        self.unfold_dim = self.unfold_range * self.unfold_range # 9
        self.cell_decode = cell_decode
        self.max_scale = 4  # Max training scale
        #TODO 是否采用Latent MLP输出split得到的LD Latent code（不采用还是会加速）
        ...
        #TODO Latent MLP接收Latent code（特征展开以及像素单元）作为输入
        if hypernet_spec is not None:
            ...
            # Latent MLP输出Render MLP每一层的Latent Modulation(scale+shift)，维度与hidden_dim相同
            self.mod_dim = 0
            ...
            # 采用Latent MLP得到的LD Latent code（16）
            if self.mod_input:
                hypernet_out_dim += imnet_spec['args']['hidden_dim'] #16
            # Latent MLP
        ...
        if imnet_spec is not None:
            ...
            if self.cell_decode: # 像素单元信息被重复使用 
                imnet_in_dim += 2
            # Render MLP
        ...
        #TODO Use CMSR in testing（cmsr_spec在配置文件中未设置）
    ...
    def gen_feats(self, inp, inp_coord=None):
        ...
        feat = self.encoder(inp) # LR feat

        if inp_coord is not None:
            self.feat_coord = inp_coord.permute(0, 3, 1, 2) # LR coord
    ...
    # 不采用CMSR（一般在训练时采用，在update_scale2mean中有引用，而update_scale2mean在测试时采用）
    ...
    # 分组快速查询（配合query_rgb_fast，每次查询多个查询点，只在测试阶段发挥作用）
