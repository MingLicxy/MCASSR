...
class Attention_regular(nn.Module):
    ...
    def __init__(self, dim, resolution, idx, split_size=[2, 4], dim_out=None, num_heads=6, attn_drop=0., proj_drop=0.,
        ...
        #TODO 矩形框[H=4,W=16] [H=16,W=4]在不同层之间交替
        elif idx == 0:
...
class RCB(nn.Module):
    ...
    #TODO 特征提取主干中的每一个基础快都是两个输入，两个并行的RCB块内做互换的交叉注意力
    def forward(self, x, Ref, x_size):
        ...
        ########################################### TODO 关键代码 ###############################################
        q = qkv[0].transpose(-2, -1).contiguous().view(B, C, H, W)  # Tar_Q 为LCM做局部特征集成做准备
        ...
        #TODO Locality Complementary Module(LCM)
        lcm = self.get_v(q) # 因为做自注意力所以qkv是相同的
...
class RCT(nn.Module):
    ...
    def __init__(self,
                 img_size=64,
                 in_chans=1,   # 这里用到的MRI图像是.png格式 3
                 embed_dim=180,
                 depth=[2, 2, 2, 2],
    ...
    def forward(self, x, Ref):
        #TODO RCT接收的输入H,W都必须被16整除
        h, w = x.shape[-2:] # 将H,W作为参数传入
        pad_h = (16 - h % 16) % 16  # 高度需要填充的总量
        pad_w = (16 - w % 16) % 16  # 宽度需要填充的总量
        pad_top = pad_h // 2
        pad_bottom = pad_h - pad_top
        pad_left = pad_w // 2
        pad_right = pad_w - pad_left
        x_padded = F.pad(x, (pad_left, pad_right, pad_top, pad_bottom), mode='reflect')
        Ref_padded = F.pad(Ref, (pad_left, pad_right, pad_top, pad_bottom), mode='reflect')

        x = self.conv_first(x_padded)
        Ref = self.conv_first(Ref_padded)
        ...
        out_padded = self.conv_before_upsample(fused) # 没有上采样

        #TODO 去除padding
        out = out_padded[:, :, pad_top:pad_top + h, pad_left:pad_left + w]
...
def make_rct(no_upsampling=True):
    return RCT(upscale=1,
               in_chans=3,
               img_size=48,  # 要求别横纵轴整除 64
               depth=[4, 4, 4, 4], # 训练慢的根源在于超参数设置 [6, 6, 6, 6]
               embed_dim=64, # 通道数 90
               num_heads=[4, 4, 4, 4],  # [6,6,6,6]
               split_size_0=[4, 4, 4, 4, 4, 4],
               split_size_1=[16, 16, 16, 16, 16, 16], # 64都可整除  测试时要求输入图像尺寸也能被整除
               mlp_ratio=2,
               resi_connection='1conv',
...
if __name__== '__main__':
    ...
    x = torch.randn(1, 3, 49, 48).cuda()
    ref = torch.randn(1, 3, 49, 48).cuda()
