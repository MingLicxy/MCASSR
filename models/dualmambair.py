...
class Dual_MambaIR(nn.Module):
    def __init__(self,
                 ...
                 mam_dim_1=64, 
                 mam_dim_2=64, # 96
                 depths= (2, 2, 2, 2), #TODO 决定FusionBlock数量 (4, 4, 4, 4)(2, 2, 2, 2)
                 ...
                 resi_connection='1conv',
                 **kwargs):
        super(Dual_MambaIR, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        ...
        self.mam_dim_1 = mam_dim_1 # 64
        self.mam_dim_2 = mam_dim_2 # 64
        self.fus_dim = fus_dim = mam_dim_1 + mam_dim_2 # 128
        #####################################################################################
        ...
        #TODO 隐藏层通道维拓展
        mam_hidden = mam_dim_1 * expansion_ratio 
        cnn_hidden = mam_dim_2 * expansion_ratio
        ...
        self.conv_first_1 = nn.Conv2d(num_in_ch, mam_dim_1, 3, 1, 1)  # 64
        self.conv_first_2 = nn.Conv2d(num_in_ch, mam_dim_2, 3, 1, 1)  # 64
        ...
        #TODO Mamba分支一：对应T2(target)
        self.mamba_branch_1 = nn.ModuleList()
        for i_layer in range(self.num_layers): # RSSG数量
            ...
            self.mamba_branch_1.append(layer)

        #TODO Mamba分支二：对应PD(reference)
        self.mamba_branch_2 = nn.ModuleList()
        for i_layer in range(self.num_layers): # RSSG数量
            layer = Mamba_RG(
                dim=mam_dim_2,
                input_resolution=(patches_resolution[0], patches_resolution[1]),
                ...
                patch_size=patch_size,
                resi_connection=resi_connection)
            self.mamba_branch_2.append(layer)

        self.norm = norm_layer(self.mam_dim_1) # 定义归一化层 dim=64
        ################################################################################
        ...
        for i_layer in range(self.num_layers): # 4
            ...
            #     FusionBlock(conv, mam_dim * 2, 1, act=nn.ReLU(True)),
            # )
            layer = FusionBlock_att(channels=mam_dim_1) #64
            self.fusion_block.append(layer)

        ## fusion_block连接mamba_branch_1
        self.fusion_mam_1 = nn.ModuleList()
        for i_layer in range(self.num_layers): # 4
            layer = nn.Sequential( 
                nn.LayerNorm(mam_dim_1),
                nn.Linear(mam_dim_1, mam_dim_1),
                nn.GELU(),
                nn.Linear(mam_dim_1, mam_dim_1),
            )
            self.fusion_mam_1.append(layer)
        
        ## fusion_block连接mamba_branch_2
        self.fusion_mam_2 = nn.ModuleList()
        for i_layer in range(self.num_layers): # 4
            layer = nn.Sequential( 
                nn.LayerNorm(mam_dim_2),
                nn.Linear(mam_dim_2, mam_dim_2),
                nn.GELU(),
                nn.Linear(mam_dim_2, mam_dim_2),
            )
            self.fusion_mam_2.append(layer)
        ################################################################################
        ...
        if resi_connection == '1conv': #TODO 最后一层Conv负责将fusion_feat的维度降低
            # self.conv_after_body = nn.Conv2d(mam_dim_1, mam_dim_1, 3, 1, 1)
            #TODO 方案二
            self.conv_after_body = nn.Conv2d(mam_dim_1 + mam_dim_2, mam_dim_1, 3, 1, 1)
        elif resi_connection == '3conv':
            self.conv_after_body = nn.Sequential(
                nn.Conv2d(mam_dim_1, mam_dim_1 // 4, 3, 1, 1), nn.LeakyReLU(negative_slope=0.2, inplace=True),
                nn.Conv2d(mam_dim_1 // 4, mam_dim_1 // 4, 1, 1, 0), nn.LeakyReLU(negative_slope=0.2, inplace=True),
                nn.Conv2d(mam_dim_1 // 4, mam_dim_1, 3, 1, 1))
        ...
        if self.upsampler == 'none': #TODO
            self.conv_before_upsample = nn.Sequential( # 180->64
                nn.Conv2d(mam_dim_1, num_feat, 3, 1, 1), nn.LeakyReLU(inplace=True))
            #BUG 方案二
            # self.conv_before_upsample = nn.Sequential( # 180->64
            #     nn.Conv2d(mam_dim_1 + mam_dim_2, num_feat, 3, 1, 1), nn.LeakyReLU(inplace=True))
            ...
            self.upsample = UpsampleOneStep(upscale, mam_dim_1, num_out_ch)
        else:
            # for image denoising
            self.conv_last = nn.Conv2d(mam_dim_1, num_out_ch, 3, 1, 1)
    ...
    #TODO 定义浅层特征提取之后的逻辑（x和ref是提取的浅层特征）
    def forward_features(self, x, ref):
        ...
        ref_mam = self.patch_embed(ref)
        #x_mam = self.pos_drop(x)

        for i in range(self.num_layers): # 4
            x_mam = self.mamba_branch_1[i](x_mam, x_size) # [bs, seq_len, c]
            ref_mam = self.mamba_branch_2[i](ref_mam, ref_size)
            x_mam_res, ref_mam_res = x_mam, ref_mam # 残差

            #TODO 配合交互融合模块做张量形状调整
            x_mam = self.patch_unembed(x_mam, x_size) # [bs, c, h, w]
            ref_mam = self.patch_unembed(ref_mam, ref_size)

            #TODO FusionBlock_att两个输入两个输出
            x_mam, ref_mam = self.fusion_block[i](x_mam, ref_mam)

            ################ TODO 最后的融合模块（add还是cat决定fus_dim）################
            if i == (self.num_layers-1):
                # 方案一：add 
                # x_fus = x_mam + ref_mam   # 64 [bs, c, h, w]

                # 方案二：cat（配合最后一层Conv输入维度翻倍）
                # x_fus = torch.cat([x_mam, ref_mam], dim=1)
                x_out = x_mam
                ref_out = ref_mam
            ###########################################################################
            ...
            ref_mam = self.patch_embed(ref_mam)

            #TODO 这里的操作是否需要可以作为变量进行研究
            x_mam = self.fusion_mam_1[i](x_mam) + x_mam_res
            ref_mam = self.fusion_mam_2[i](ref_mam) + ref_mam_res
    

        #BUG 是否需要这个归一化层（只接受[bs, seq_len, c]）
        x_out = x_out.permute(0, 2, 3, 1) # [bs, c, h, w]->[bs, h, w, c]
        ref_out = ref_out.permute(0, 2, 3, 1)
        x_out = self.norm(x_out).permute(0, 3, 1, 2) # [bs, h, w, c]->[bs, c, h, w] 
        ref_out = self.norm(ref_out).permute(0, 3, 1, 2)
        x_fus = torch.cat([x_out, ref_out], dim=1) # 先各自归一化再cat，通道维加倍
        return x_fus
    ...
    def forward(self, x, ref):
        ...
        if self.upsampler == 'none':
            ...
            ref = self.conv_first_2(ref)
            #TODO 融合PD与T2的目的在于补充T2缺失的信息
            x = self.conv_after_body(self.forward_features(x, ref)) + x 
            #TODO 上采样层之前的最后一层Conv
