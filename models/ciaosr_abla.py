...
原始CiaoSR在基于注意力机制进行特征聚合之前通过投影层融合特征、相对坐标以及尺度信息
CiaoSR_Abla将相对坐标和尺度信息移除, 再在最终解码过程中进行cat输入
"""
...
################################ CiaoSR_Abla ##################################
@register('ciaosr_abla')
class CiaoSR_Abla(nn.Module):
    """
    ...
    def __init__(self,
                 ...
                 multi_scale=[2],
                 softmax_scale=1,
                 coord_decoder=True, # 计算KV投影是否考虑coord/scale
                 scale_decoder=True
                 ):
        super().__init__()
        ...
        # 消融相关
        self.coord_decoder = coord_decoder
        ...
        #TODO 考虑相对坐标与像素尺度
        if self.coord_decoder:
            imnet_k_in_dim += 2
            imnet_v_in_dim += 2
        # else: # 投影计算不考虑coord/scale，最后解码过程就要考虑
        #     imnet_q_in_dim += 2 
        if self.scale_decoder:
            imnet_k_in_dim += 2
            imnet_v_in_dim += 2
        # else:
        #     imnet_q_in_dim += 2
    ...
    def query_rgb(self, coord, scale=None):
        ...
        for feature in self.feats: #TODO 遍历的目的？
            ...
            #TODO 3，采用双三次插值
            # query = F.grid_sample(feat_q, coord.flip(-1).unsqueeze(1), mode='bicubic',
            #                          align_corners=False).permute(0, 3, 2, 1).contiguous()
            #TODO 4，采用LIIF实现
            ...
            for v in v_lst:
                ...
                #TODO 消融点 相对坐标用于计算K,V（这里采用了位置信息）
                if self.coord_decoder and self.scale_decoder:
                    inp_v = torch.cat([value, inp, scale_], dim=-1)  # [16, 2304, 580] 64x9+2+2=580
                    inp_k = torch.cat([key, inp, scale_], dim=-1)  # [16, 2304, 580]
                if self.coord_decoder and not self.scale_decoder:
                    inp_v = torch.cat([value, inp], dim=-1) 
                    inp_k = torch.cat([key, inp], dim=-1)  
                if not self.coord_decoder and self.scale_decoder:
                    inp_v = torch.cat([value, scale_], dim=-1)  
                    inp_k = torch.cat([key, scale_], dim=-1)  
                if not self.coord_decoder and not self.scale_decoder:
                    inp_v = torch.cat(value, dim=-1)  
                    inp_k = torch.cat(key, dim=-1)
        ...
        #TODO 对聚合的特征进行最终解码，out_dim=3的imnet_q
