...
class Mam_Mam_1(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = self.patch_embed(x) # [bs, seq_len, c]
        #x_mam = self.pos_drop(x)

        for i in range(self.num_layers): # 4
            x_mam = self.mamba_branch[i](x, x_size) # [bs, seq_len, c]
            x_cnn = self.cnn_branch[i](x, x_size) # [bs, seq_len, c]
            x_mam_res, x_cnn_res = x_mam, x_cnn # 残差

            #TODO x_mam与x_cnn的形状不同
            x_mam = self.patch_unembed(x_mam, x_size) # [bs, c, h, w]
            x_cnn = self.patch_unembed(x_cnn, x_size) # [bs, c, h, w]
            ...
            x_cnn = self.patch_embed(x_cnn) # [bs, seq_len, c]
