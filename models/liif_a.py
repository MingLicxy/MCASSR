...
class LIIF_A(nn.Module):
    ...
    def query_rgb(self, coord, cell=None):
        ...
        #TODO 特征图展开，以便更好地捕获局部特征（以每一个像素坐标为中心的3x3区域）
        if self.feat_unfold:
        ...
        for vx in vx_lst:
            for vy in vy_lst:
                ...
                # 自适应权重
                if self.use_pdn:
                    Coeff=self.pdn(inp) # out:(bs,h*w,K) K个权重对应K个MLP
                else:
                    Coeff=torch.ones([inp.shape[0],inp.shape[1],1]) # (bs,q,1)
