...
##################################------MambaIR------#################################
class Mamba_CNN_NO_Fusion(nn.Module):
    def __init__(self,
                 conv = default_conv, #TODO 默认卷积
                 img_size=48,
                 patch_size=1,
                 in_chans=3,
                 mam_dim=64, # mamba_branch中间层特征通道维 60
                 cnn_dim=64, # cnn_branch中间层特征通道维
                 depths=(2, 2, 2, 2), #TODO 决定FusionBlock数量
                 ...
                 resi_connection='1conv',
                 **kwargs):
        super(Mamba_CNN_NO_Fusion, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        ...
        self.norm = norm_layer(self.mam_dim + self.cnn_dim) # 定义归一化层
        ...
        if resi_connection == '1conv': #TODO 最后一层Conv负责将fusion_feat的维度降低
            self.conv_after_body = nn.Conv2d(fus_dim, mam_dim, 3, 1, 1)
        elif resi_connection == '3conv':
            self.conv_after_body = nn.Sequential(
                nn.Conv2d(fus_dim, fus_dim // 4, 3, 1, 1), nn.LeakyReLU(negative_slope=0.2, inplace=True),
                nn.Conv2d(fus_dim // 4, fus_dim // 4, 1, 1, 0), nn.LeakyReLU(negative_slope=0.2, inplace=True),
                nn.Conv2d(fus_dim // 4, mam_dim, 3, 1, 1))
    ...
    def forward_features(self, x):
        ...
        x_mam = self.patch_unembed(x_mam+x_mam_res, x_size) # [bs, c, h, w]
        x_cnn = x_cnn + x_cnn_res # # [bs, c, h, w]
        
        #TODO 双分支融合方案
        # 方案一：add（fus_dim=64）
        #x_fus = x_mam + x_cnn
        # 方案二：cat（fus_dim=128）
        x_fus = torch.cat((x_mam, x_cnn), 1) # 沿通道维cat
            ...
            # if i != (self.num_layers-1): #TODO 前(n-1)个融合模块
            #     #TODO 
            #     x_mam, x_cnn = torch.split(x_fus, [self.mam_dim, self.cnn_dim], 1) # [bs, c, h, w]
        ...
        #注意：patch_embed()与patch_unembed()内部有维度为64的norm
        #x_fus = self.patch_embed(x_fus) # [bs, c, h, w]->[bs, seq_len, c]
        #x = self.norm(x_fus)
        #x = self.patch_unembed(x) # [bs, seq_len, c]->[bs, c, h, w]
...
@register('mamba_cnn_no_fusion')
def make_mamba_cnn_no_fusion(no_upsampling=True):
    return Mamba_CNN_NO_Fusion()
...
if __name__== '__main__':
    ...
    model = Mamba_CNN_NO_Fusion().cuda()
    ...
    model = Mamba_CNN_NO_Fusion().eval().cuda()
