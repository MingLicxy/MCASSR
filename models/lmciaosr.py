...
class LMCiaoSR(nn.Module):
    def __init__(self,
        ...
        #TODO Convert 576-dim(64x9) query to 256-dim(964x4) query
        prenet_q_in_dim = self.encoder.out_dim * self.unfold_dim # 3x3
        prenet_q_out_dim = self.encoder.out_dim * (real_unfold_dim if self.feat_unfold else 1) #2x2
        # 特征维度收缩
    ...
    #TODO 将计算kv以及隐式调制mod的过程集成为函数（包含原始CiaoSR完整逻辑）
    def query_latent(self, feat, scale=None):
        ...
        #TODO Query modulations CiaoSR核心逻辑
        attn = (query @ preds_k)
    ...
    def query_rgb(self, feat, coord, scale=None):
        ...
        for v in v_lst:
            ...
            if self.training:
                #TODO self.mod是应用query_latent()计算得到
                # query_latent()包含原始CiaoSR中完整逻辑
                q_mod = F.grid_sample(
                    self.mod, coord_.flip(-1).unsqueeze(1),
                    mode='nearest', align_corners=False)[:, :, 0, :] \
                    .permute(0, 2, 1).contiguous().view(bs * q, -1)
                
                #TODO 用原始CiaoSR生成的mod来调制LIIF逻辑中的输出
        ...
        #TODO 采用LIIF的局部聚合策略（在原始CiaoSR中不存在）
